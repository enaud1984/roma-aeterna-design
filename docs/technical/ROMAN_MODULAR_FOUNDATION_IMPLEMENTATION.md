...
## Aggiornamento Prompt 21 — visualizzazione runtime

La fondazione ora include un adapter visuale runtime in `ARARomanProceduralBuildingActor`. I placement astratti restano prodotti dal core e dalla rule library; la conversione in placeholder tecnici avviene solo nell'actor tramite primitive Engine, `UInstancedStaticMeshComponent` e batching per categoria/mesh.

Stati: `PLACEHOLDER_RUNTIME_STATIC_CHECKS_PASSED`, `FAB_ASSET_IMPORT_NOT_STARTED`, `UNREAL_BUILD_REQUIRED`, `UNREAL_AUTOMATION_REQUIRED`, `MANUAL_VERIFICATION_REQUIRED`.
