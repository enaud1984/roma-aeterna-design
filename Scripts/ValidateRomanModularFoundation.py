...
for fn in 'GenerateBuilding ClearGeneratedBuilding RebuildBuilding GenerateFromParameters BuildVisualInstances ClearVisualInstances GetGeneratedInstanceCount GetInstanceCountByCategory HasGeneratedBuilding'.split():
    if fn not in allcpp: errors.append(f'API runtime placeholder mancante: {fn}')
actor_text=need('Source/RomaAeterna/Private/World/Modular/RARomanProceduralBuildingActor.cpp') + need('Source/RomaAeterna/Public/World/Modular/RARomanProceduralBuildingActor.h')
for token in ['UInstancedStaticMeshComponent','/Engine/BasicShapes/Cube.Cube','/Engine/BasicShapes/Cylinder.Cylinder','/Engine/BasicShapes/Sphere.Sphere','/Engine/BasicShapes/Cone.Cone','TSoftObjectPtr<UStaticMesh>']:
    if token not in actor_text: errors.append(f'placeholder runtime mancante: {token}')
...
for state in ['CORE_CPP_DEBUG_TESTS_PASSED','CORE_CPP_RELEASE_TESTS_PASSED','CORE_CPP_SANITIZERS_PASSED','UNREAL_BUILD_REQUIRED','UNREAL_AUTOMATION_REQUIRED','MANUAL_VERIFICATION_REQUIRED','PLACEHOLDER_RUNTIME_STATIC_CHECKS_PASSED','FAB_ASSET_IMPORT_NOT_STARTED']:
...
for scanned in root.rglob('*'):
    if scanned.is_file() and '.git' not in scanned.parts and not any(part.startswith('build-cloud') for part in scanned.parts) and scanned.suffix.lower() not in ['.png','.jpg','.jpeg','.pdf','.docx','.uasset','.umap']:
        data=scanned.read_text(encoding='utf-8', errors='ignore')
        if re.search(r'^(<<<<<<<|=======|>>>>>>>)', data, re.MULTILINE):
            errors.append(f'marcatore conflitto Git in {scanned}')
