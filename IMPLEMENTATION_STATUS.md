...
## Prompt 21 — Generatore visuale runtime con placeholder modulari

Stato: implementazione runtime e controlli cloud completati.

`ARARomanProceduralBuildingActor` trasforma i placement astratti in placeholder visibili usando primitive Engine con soft reference, batching tramite `UInstancedStaticMeshComponent`, conteggi per categoria, API Blueprint di generazione/cancellazione/rigenerazione e debug opzionale. Nessun asset Fab è stato importato.

Esiti cloud: `CORE_CPP_DEBUG_TESTS_PASSED`, `CORE_CPP_RELEASE_TESTS_PASSED`, `CORE_CPP_SANITIZERS_PASSED`, `PLACEHOLDER_RUNTIME_STATIC_CHECKS_PASSED`, `FAB_ASSET_IMPORT_NOT_STARTED`. Restano locali: `UNREAL_BUILD_REQUIRED`, `UNREAL_AUTOMATION_REQUIRED`, `MANUAL_VERIFICATION_REQUIRED`.
