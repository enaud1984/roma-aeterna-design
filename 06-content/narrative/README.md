# Narrazione e Dialoghi

## Scopo

Definire l'architettura documentale del dominio **Narrazione e Dialoghi**.

## Descrizione

Indice delle specifiche modulari da approfondire dopo l'approvazione dell'architettura.

## Ambito

Include i documenti elencati; implementazione e asset sono esclusi.

## Documenti contenuti

- [Architettura Narrativa](narrative-architecture.md)
- [Narrazione Emergente](emergent-narrative.md)
- [Storylet](storylets.md)
- [Sistema di Dialogo](dialogue-system.md)
- [Authoring dei Dialoghi](dialogue-authoring.md)
- [Lingue](languages.md)
- [Onomastica](naming.md)
- [Continuità Narrativa](narrative-continuity.md)
- [Sensibilità dei Contenuti](content-sensitivity.md)

## Dipendenze

- [Visione creativa](../../01-vision/creative-vision.md)
- [Standard documentale](../../00-governance/documentation-standard.md)

## Collegamenti con le altre aree

- [Indice generale](../../README.md)
- [Mappa documentale](../../00-governance/documentation-map.md)
- [Matrice delle dipendenze](../../00-governance/system-dependency-matrix.md)

## Stato di avanzamento

**Stato:** narrazione emergente e memoria causale a S3; dialoghi/storylet attendono corpus e budget P0.

## Decisioni ancora aperte

- Ownership, priorità, scope della demo e profondità finale.

## TODO

- Approvare l'indice e assegnare owner/reviewer.
- Collegare fonti, ADR, test e Definition of Done.
