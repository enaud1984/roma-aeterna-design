# Progettazione della Demo di Pompei

## Scopo

Definire l'architettura documentale del dominio **Progettazione della Demo di Pompei**.

## Descrizione

Indice delle specifiche modulari da approfondire dopo l'approvazione dell'architettura.

## Ambito

Include i documenti elencati; implementazione e asset sono esclusi.

## Documenti contenuti

- [Dossier urbano integrato](pompeii-urban-system.md)
- [Data e snapshot](historical-date.md)
- [Area giocabile](playable-area.md)
- [Zone e flussi](districts.md)
- [Edifici accessibili](accessible-buildings.md)
- [Cicli e calendario](daily-seasonal-cycles.md)
- [Rete regionale](regional-connections.md)

- [Mandato della Demo](demo-charter.md)
- [Data Storica della Demo](historical-date.md)
- [Area Giocabile](playable-area.md)
- [Quartieri della Demo](districts.md)
- [Edifici Accessibili](accessible-buildings.md)
- [Cicli di Pompei](daily-seasonal-cycles.md)
- [Personaggi Iniziali](starting-characters.md)
- [Professioni della Demo](demo-professions.md)
- [Scope dei Sistemi](demo-systems-scope.md)
- [Scope dei Contenuti](demo-content-scope.md)
- [Criteri di Accettazione della Demo](demo-acceptance.md)

## Dipendenze

- [Visione creativa](../../01-vision/creative-vision.md)
- [Standard documentale](../../00-governance/documentation-standard.md)

## Collegamenti con le altre aree

- [Indice generale](../../README.md)
- [Mappa documentale](../../00-governance/documentation-map.md)
- [Matrice delle dipendenze](../../00-governance/system-dependency-matrix.md)

## Stato di avanzamento

**Stato:** struttura creata; contenuti da sviluppare.

## Decisioni ancora aperte

- Ownership, priorità, scope della demo e profondità finale.

## TODO

- Approvare l'indice e assegnare owner/reviewer.
- Collegare fonti, ADR, test e Definition of Done.
