# Template

## Scopo

Definire l'architettura documentale del dominio **Template**.

## Descrizione

Indice delle specifiche modulari e dei registri da sviluppare nelle fasi successive.

## Ambito

Include i documenti elencati; non autorizza implementazione, asset o ampliamenti di scope.

## Documenti contenuti

- [Template decision record](decision-record-template.md)
- [Template specifica di sistema](system-spec-template.md)
- [Template provenienza storica](historical-source-template.md)
- [Template di Specifica Città](city-template.md)
- [Template NPC](npc-template.md)
- [Template Feature](feature-template.md)
- [Template Contenuto](content-template.md)
- [Template Rischio](risk-template.md)
- [Template Piano di Test](test-plan-template.md)
- [Template Fonte Storica](source-entry-template.md)
- [Template Verbale](meeting-record-template.md)

## Dipendenze

- [Visione creativa](../../01-vision/creative-vision.md)
- [Standard documentale](../../00-governance/documentation-standard.md)

## Collegamenti con le altre aree

- [Indice generale](../../README.md)
- [Mappa documentale](../../00-governance/documentation-map.md)
- [Matrice delle dipendenze](../../00-governance/system-dependency-matrix.md)

## Stato di avanzamento

**Stato:** struttura creata; contenuti da sviluppare.

## Decisioni ancora aperte

- Ownership, priorità e collocazione nelle milestone.

## TODO

- Approvare l'indice e assegnare owner/reviewer.
- Collegare ADR, rischi, test e criteri di completamento.
